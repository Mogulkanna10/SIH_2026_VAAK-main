...
COMMAND_WHITELIST = {
    ...
    "START_PROCEDURE": r"(?i).*\b(start|begin)\b.*\b(procedure|process)\b.*",
    "END_PROCEDURE": r"(?i).*\b(end|finish|complete)\b.*\b(procedure|process)\b.*",
    ...
    "EMERGENCY_STOP": r"(?i).*\b(emergency|e-stop)\b.*\b(stop|halt)\b.*"
}
